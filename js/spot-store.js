...
function writeCustomSpots(spots, { notify = false } = {}) {
    const cleaned = spots.map(normalizeCustomSpot).filter(Boolean);
...
export function getCustomSpots({ includeArchived = false } = {}) {
    const spots = readList(CUSTOM_SPOTS_KEY).map(normalizeCustomSpot).filter(Boolean);
    return includeArchived ? spots : spots.filter(spot => !spot.archived);
}

export function getAllSpots({ includeArchived = false } = {}) {
    const presets = getPresetSpots();
    // 非表示の標準ピン上書きも先に統合し、元の標準ピンが復活しないよう最後に絞り込む。
    const custom = getCustomSpots({ includeArchived: true });
    ...
    return [...mergedPresets, ...added].filter(spot => includeArchived || !spot.archived);
}
...
export function addCustomSpot(values) {
    ...
    const spot = normalizeCustomSpot({ ...values, id: makeId(), createdAt: now, updatedAt: now, source: 'custom' });
    if (!spot) throw new Error('スポット情報が不正です。');
    const spots = getCustomSpots({ includeArchived: true });
    spots.push(spot);
...
export function updateCustomSpot(id, updates) {
    ...
    const updated = normalizeCustomSpot({ ...spots[index], ...updates, id, updatedAt: new Date().toISOString() });
...
export function saveSpotChanges(id, updates) {
    ...
    const existing = getCustomSpots({ includeArchived: true }).find(spot => spot.id === id);
...
export function stashGuestSpots() {
    ...
    localStorage.setItem(GUEST_SPOTS_KEY, JSON.stringify(getCustomSpots({ includeArchived: true })));
}

export function getGuestSpots() {
    return readList(GUEST_SPOTS_KEY).map(normalizeCustomSpot).filter(Boolean);
}
